...
3. Run merge
   ...
   mergekit-yaml ../configs/merge.yaml /workspace/models/merged --cuda
   ```
...
6. Login to HuggingFace via uvx
   ```
   uvx hf login --token hf_your_token_here
   ```

7. Create a private repo and upload
   ```
   uvx hf repo create your-org/cognitive-core-v1 --type model --private
   uvx hf upload your-org/cognitive-core-v1 \
